<?php
/**
 * Helper để tạo PDF hợp đồng - sử dụng lại logic từ generate_contract_pdf.php
 * Không echo output, chỉ return PDF binary data
 */

class ContractPDFGenerator {
    
    /**
     * Tạo PDF hợp đồng từ contract data
     * 
     * @param int $contract_id ID hợp đồng
     * @return array ['success' => bool, 'data' => string|null, 'message' => string]
     */
    public static function generatePDF($contract_id) {
        try {
            // Load model để lấy thông tin hợp đồng
            ...
            if ($result['status'] !== 'success') {
                return [
                    'success' => false,
                    'data' => null,
                    'message' => 'Không tìm thấy hợp đồng: ' . $result['message']
                ];
            }
            ...
            $html = self::getContractHTML($contract);
            
            // Xuất HTML ra PDF
            $pdf->writeHTML($html, true, false, true, false, '');
            
            // Trả về PDF binary data
            return [
                'success' => true,
     ...
     */
    private static function getContractHTML($contract) {
        return '
<style>
...
<div class="section">
    ...
    <table class="info-table">
        ...
        <tr>
            ...
            <td class="value">' . date('d/m/Y', strtotime($contract['start_date'])) . '</td>
        </tr>
        <tr>
            <td class="label">Ngày kết thúc:</td>
            <td class="value">' . date('d/m/Y', strtotime($contract['registration_end_date'])) . '</td>
        </tr>
        <tr>
